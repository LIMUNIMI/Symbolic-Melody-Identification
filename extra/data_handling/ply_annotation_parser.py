...
class AnnotationTokenizer(object):
    ...
    t_FFF = r'fff'
    t_A = r'a'
    t_ACCELERANDO = r'(accelerando|(accel|acc)\.?)'
    t_ADAGIO = r'adagio'
    t_AGITATO = r'agitato'
    t_ALLA = r'alla'
...
#    t_PP = r'pp'
    ...
    def t_error(self, t):
        ...
        if self.on_error == 'skip':
            raise TokenizeException(u"Skipping illegal character '{}' at position {} in \"{}\""
                                    .format(t.value[0], t.lexpos, s).encode('utf8'))
            t.lexer.skip(1)
        else:
            raise TokenizeException(u"Illegal character '{}' at position {} in \"{}\""
                                    .format(t.value[0], t.lexpos, s).encode('utf8'))
...
def p_pp(p):
    ''' pp : p np
    '''
    p[0] = ConstantTempoDirection(u'{}_{}'.format(p[1], p[2]).lower())
...
def p_error(p):
    if p is not None:
        LOGGER.warning(u"Syntax error at '{}' in '{}'"
...
def parse_words(words):
    ...
    try:
        ...
        LOGGER.warning(u'Cannot tokenize: "{}"'.format(lwords).encode('utf8'))
        r = None

    if isinstance(r, Direction):
        r.raw = lwords
    elif r is None:
        LOGGER.warning(u'Cannot not convert "{}" into a direction'.format(lwords))
        r = Words(words)
    return r
...
def main():
    ...
    annotations = [l.strip() for l in open(args.annotations).read().decode('utf8').split(u'\n')
    ...
    for words in annotations:
        ltest = words.lower()
        print('')
        print(u'parsing "{}"'.format(ltest).encode('utf8'))
        # lex.input(ltest)
        # toktypes = []
        # while True:
        #     tok = lex.token()
        ...
        try:
            ...
            print(u'cannot tokenize: "{}"'.format(
                ltest.lower()).encode('utf8'))
            continue
        if isinstance(r, Direction):
            r.raw = words
        print(u'correctly parsed into: "{}"'.format(r).encode('utf8'))
        # return True
...
# for t in annotation.split():
# tokens.append(u'{}'.format(t.upper()))
# rules.append(u"t_{} = r'{}'".format(t.upper(), t.lower()))
...
#                 break
#             toktypes.append(tok.type)
#         tokstrings.append(' '.join(toktypes))
#     tokstrings, idx = np.unique(tokstrings, return_index=True)

#     for ts, i in zip(tokstrings, idx):
#         print(annotations[i].encode('utf8'))
#         print(ts)
# r = yacc.parse(annotations[i].lower())
# print(r)
#         print('')
