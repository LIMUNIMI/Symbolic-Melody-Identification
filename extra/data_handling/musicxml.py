...
class ScorePartBuilder(object):
    ...
    def finalize(self):
        ...
        for obj in self.ongoing.values():
            ...
            for o in obj_list:
                LOGGER.warning(u'ongoing object "{}" until end of piece'.format(o))
                end_pos = tp
                if isinstance(o, DynamicTempoDirection):
                    ct = o.start.get_next_of_type(ConstantTempoDirection)
                    if any(c.text == u'a_tempo' for c in ct):
                        end_pos = ct[0].start
    ...
    def _do_direction(self, e, measure_position):
        ...
        if len(sounds) > 0:
            if sounds[0].attrib.has_key('fine'):
                starting_directions.append(Fine())
            if sounds[0].attrib.has_key('dacapo'):
        ...
        for dts in direction_types:
            ...
            if direction_type == 'dynamics':
                ...
                for dt in dts:
                    ...
                    parse_result = parse_words(unicode(dt.text))
    ...
    def _do_note(self, e, measure_position, prev_note_start_dur):
        ...
        if len(e.xpath('notations')) > 0:
            ...
            eslurs.sort(key=lambda x: int(x.attrib['number']) if x.attrib.has_key('number') else 1)
...
def parse_parts(document, score_part_dict):
    ...
    for j in range(n_measures):
        ...
        positions = np.array([(v, k) for k, v in position_dict.items()])
        # print(positions)
