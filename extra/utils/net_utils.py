...
def read_varlen_msg(connection):
    ...
    if len(msg) < msg_len:
        logger.info('connection closed before message could be read {}/{} bytes'.format(len(msg), msg_len))
        return None
    ...
    try:
        while True:
            ...
            if only_allow_from and (client_address[0] not in only_allow_from):
                logger.info('rejecting unauthorized connection from {}'.format(client_address[0]))
                connection.close()
                continue
            else:
                logger.info('client is authorized: {}'.format(client_address[0]))
